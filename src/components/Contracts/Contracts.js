...
class Contracts extends React.Component {
  ...
  newContract(trx) {
    if (trx) this.setState({ pendingTrx: trx });
  }
  ...
  render() {
    function SelectedContract(props) {
      if (props && props.contract) {
        return (
          <ContractData
            settlerContracts={props.settlerContracts}
            onLeased={props.onLeased}
            onError={props.onError}
            changeRent={props.changeRent}
      ...
      if (props.pendingTrx) {
        return (
          <Alert bsStyle="success" onDismiss={props.handleDismiss}>
            ...
            <p>
              The transaction is currently pending and waiting for
              confirmations...{' '}
              <a
                target="_blank"
                href={`https://ropsten.etherscan.io/tx/${props.pendingTrx}`}
              >
                Click here to see more details about the transaction.
              </a>
            </p>
          </Alert>
        );
    ...
    function ContractList(props) {
      ...
      const firstItemIndex = props.itemsPerPage * (props.activePage - 1);
      const items = props.contracts.slice(
        firstItemIndex,
        firstItemIndex + props.itemsPerPage,
      );
    ...
    function Pager(props) {
      if (props.nbItems <= 0) return null;
          ...
          <div className="panel-heading">
            <h4>
              Spawn Contracts<div className="pull-right">
                <NewContract onCreate={this.newContract.bind(this)} />
              </div>
            </h4>{' '}
          </div>
          <div className="panel-body">
            <div className="row">
              <div className="col-md-5">
                <table className="table">
                  ...
                  <ContractList
                    ...
                    handleClick={this.selectContract.bind(this)}
                  />
                </table>
                <Pager
                  nbItems={this.props.contracts.length}
              ...
              <div className="col-md-7">
                <SelectedContract
                  settlerContracts={this.props.contracts.filter(
                    c => c.contractState === '6',
                  )}
                  onLeased={this.handleOnLeased.bind(this)}
                  onError={this.handleOnError.bind(this)}
                  changeRent={this.handleChangeRent.bind(this)}
